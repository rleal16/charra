...
mbedtls_rsa_context load_key(void)
{
    ...
    mbedtls_printf( "\n  . Reading public key from the file rsa_pub.txt" );
    fflush( stdout );
...
int ra_iot_encrypt( mbedtls_rsa_context *rs, unsigned char input[])
{
    //printf("RSA Key verifications: %d\n", mbedtls_rsa_check_pubkey(rsa));
    #if defined(MBEDTLS_PKCS1_V15)
    printf("MBEDTLS_PKCS1_V15 is defined!!");
    #endif

    #if defined(MBEDTLS_PKCS1_V21)
    printf("MBEDTLS_PKCS1_V21 is defined!!");
    #endif
    FILE *f;
    int ret = 1;
    int exit_code = 0;
